...
async def get_user_by_id(user_id: str) -> Optional[User]:
    if user_manager:
        try:
            user_dict = await user_manager.get_user_by_id(user_id)
...
async def ensure_admin_user():
    ...
    if not admin_exists:
        admin_data = UserRegistration(
            ...
            username="admin",
            password=os.getenv('DEFAULT_ADMIN_PASSWORD', 'TempPass123!'),
            role=UserRole.ADMIN
        )
        admin_user = await create_user(admin_data)
        logger.info(f"Created default admin user: {admin_user.email}")
        logger.warning("Change the default admin password in production!")

async def create_or_update_oauth_user(email: str, display_name: str, provider: str, provider_id: str, avatar_url: str = None) -> User:
    if user_manager:
        try:
            user_dict = await user_manager.create_user_from_oauth(
                email=email,
...
def fetch_apple_public_keys() -> Dict[str, Any]:
    ...
    try:
        ...
        _apple_keys_cache["expires"] = current_time + 3600
