"""
Unified Storage Helper for API Gateway
Automatically selects between S3, Azure, or local storage based on configuration.
"""
...
STORAGE_BACKEND = os.getenv('STORAGE_BACKEND', 'auto').lower()  # auto | s3 | azure | local


def get_book_files_directory() -> str:
    """
    Get the book files directory path.
    Checks environment variable first, then Docker path, then development path.
    """
    ...
    # Development path - go up from api_gateway to project root
...
def get_fallback_local_path(book_folder: str, filename: str) -> Optional[str]:
    """
    Get local file path as fallback when cloud storage is unavailable.
...
class StorageHelper(Protocol):
    """Protocol defining the storage helper interface"""
    enabled: bool
...
class LocalStorageHelper:
    """Fallback helper for local filesystem storage"""

    def __init__(self):
        self.enabled = False  # Cloud storage not enabled
        self.book_files_dir = get_book_files_directory()
        logger.info(f"Local storage initialized: {self.book_files_dir}")

    def generate_audio_url(self, book_folder: str, filename: str) -> Optional[str]:
        """Local files are served via API endpoint, return None to trigger fallback"""
        return None
    ...
    def upload_cover_image(self, local_file_path: str, target_path: str) -> bool:
        """Copy cover image within local filesystem"""
        return self.upload_audio_file(local_file_path, target_path)

    def get_stream_url(self, file_path: str, expiry_hours: int = 1) -> Optional[str]:
        """Local files are served via API endpoint"""
        return None

    def list_book_audio_files(self, book_folder: str) -> List[str]:
        """List audio files in local directory"""
...
def get_storage_helper() -> StorageHelper:
    """
    Factory function to get the appropriate storage helper.

    Priority when STORAGE_BACKEND=auto:
    1. S3 (if AWS_S3_BUCKET_NAME is configured)
    2. Azure (if AZURE_STORAGE_ACCOUNT_NAME is configured)
    ...
    # Auto-detect mode
    # Try S3 first (preferred for AWS deployment)
    try:
        from s3_storage_helper import S3StorageHelper
...
def get_storage() -> StorageHelper:
    """Get the singleton storage helper instance"""
